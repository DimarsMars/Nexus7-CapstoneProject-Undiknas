...
class _MyEditProfileScreen extends State<EditProfileScreen> {
      ...
      if (mounted) {
        setState(() {
          ...
          if (profile.description != null &&
            ...
            final categoryNames = profile.description!
                .split(',')
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      // --- Perbaikan Alur Photo Profile ---
      // Setelah berhasil update, reset _imageFile dan muat ulang data profil
      // dari API untuk memastikan tampilan sesuai dengan data terbaru di backend.
      if (mounted) {
        setState(() {
          _imageBytes = null; // Clear the temporary picked image
          _pickedXFile = null;
        });
        await _loadProfileData(); // Reload _profile to get the updated photo (Base64 string)
      }
      // --- Akhir Perbaikan ---
                            ...
                            children: [
                              Stack(
                                ...
                                children: [
                                  ...
                                  Text(
                                    (_profile!.rank.isNotEmpty &&
                                            _profile!.rank.contains('lvl'))
                                        ? _profile!.rank
                                            .split(' ')
                                            .last
                                            .replaceAll('lvl', '')
  ...
  Widget _buildTextField({
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        hintText: hint,
        hintStyle:
            const TextStyle(color: _darkBlue, fontWeight: FontWeight.bold),
                ...
                Expanded(
                  child: _selectedCategories.isEmpty
                      ? const Text(
                          'Select Preference',
                          style: TextStyle(
                              color: _darkBlue, fontWeight: FontWeight.bold),
                        )
                      : Wrap(
                          spacing: 6.0,
                          runSpacing: 6.0,
        ...
        if (_isDescriptionDropdownOpen)
          Container(
            ...
            child: availableCategories.isEmpty
                ? const Center(
                    child: Text('All categories selected or none available.'))
                : ListView.builder(
