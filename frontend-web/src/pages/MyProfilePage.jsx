...
const MyProfilePage = () => {
    ...
    // 3. FETCH MY REVIEWS (Reviews I gave)
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchReviewsOnMyPlans = async () => {
            ...
            try {
                // Pastikan nama function ini ada di apiService kamu
                const response = await apiService.getReviewsOnMyPlans(); 
                ...
                setReviewsOnMyPlans(data);
            } catch (error) {
                console.error("Error fetching reviews on my plans:", error);
            }
        };
    ...
    return (
        <div className="min-h-screen bg-gray-100 py-10 px-5 pt-30 flex justify-center items-start md:items-center">
            <div className="bg-white w-full max-w-7xl p-6 md:p-10 rounded-xl shadow-sm">
                    ...
                    <div className="shrink-0">
                        <div className="w-32 h-32 md:w-45 md:h-45 rounded-full overflow-hidden border-4 border-gray-200 shadow-lg bg-gray-300">
                            {profileImage ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center text-gray-400">Profile</div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {myPlans && myPlans.length > 0 ? (
                            ...
                            <div className="md:col-span-2 flex flex-col items-center justify-center py-10 text-gray-500">
                                ...
                                <button onClick={() => navigate('/maps')} className="mt-4 px-6 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700 transition shadow-sm tracking-wide">
                                    Buat Rute Baru
                                </button>
                            </div>
                        )}
                ...
                {/* SECTION YOUR REVIEW */}
                <div className="pt-15">
                    <h2 className="text-lg md:text-xl font-bold text-start text-slate-900 mb-2">Your review</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {myReviews.length > 0 ? (
                ...
                {/* SECTION THOSE WHO REVIEW YOU (INTEGRATED) */}
                <div className="pt-15 text-start">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {reviewsOnMyPlans.length > 0 ? (
                            ...
                            <div className="md:col-span-3 flex flex-col items-center justify-center py-10 text-gray-500">
